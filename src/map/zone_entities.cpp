...
void CZoneEntities::DecreaseZoneCounter(CCharEntity* PChar)
{
	...
	//remove bcnm status
	if (m_zone->m_BattlefieldHandler != nullptr && (PChar->StatusEffectContainer->HasStatusEffect(EFFECT_BATTLEFIELD) || PChar->StatusEffectContainer->HasStatusEffect(EFFECT_DYNAMIS)))
	{
        luautils::OnBcnmLeave(PChar, PChar->PBCNM, LEAVE_WARPDC);
	}
